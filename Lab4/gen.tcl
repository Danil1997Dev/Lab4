...
# File: gen.tcl
# Generated on: Mon Jan 24 18:04:30 2022
...
if {[is_project_open]} {
	if {[string compare $quartus(project) "gen"]} {
		puts "Project gen is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists gen]} {
		project_open -revision gen gen
	} else {
		project_new -revision gen gen
	}
...
if {$make_assignments} {
	...
	set_global_assignment -name TOP_LEVEL_ENTITY gen
	...
	set_global_assignment -name VERILOG_FILE sdmodb.v
	set_global_assignment -name VERILOG_FILE phacc.v
	set_global_assignment -name VERILOG_FILE gen.v
